...
Главное правило: нейроперсона и AI генерируют гипотезы, а не подтверждённые факты. Если пользователь просит применить методологию к своему продукту, явно разделяй: «что известно из его данных», «рабочая гипотеза» и «что проверить на реальных людях».
Не выдумывай отзывы, интервью, цифры, источники или пользовательское поведение. Для высокорисковых доменов (здоровье, деньги, безопасность) напомни, что перед продуктовым решением нужны первичные данные, профильная экспертиза и safety gate.
...
6. Отдельно маркируй: «подтверждено данными», «гипотеза», «требует валидации». Никогда не выдавай предположение за факт.
7. Если артефакт относится к B2B, проверь наличие разных ролей (Buyer, Admin, Team Lead, End User) и конфликтов их интересов. Если сценарий связан с высокой эмоцией — проверь стресс, социальное давление и цену публичной ошибки. Если домен высокорисковый (здоровье, финансы, безопасность) — требуй первичные данные, профильного эксперта и safety gate до продуктовой рекомендации.
